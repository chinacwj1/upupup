<!DOCTYPE HTML>
<html>
    <head>
        <meta charset="UTF-8">
        <title>召唤师 博文</title>
        <body background="<?php echo base_url().'\bg\background1.jpg'?>">
        <link rel="stylesheet" type="text/css" href="<?php echo base_url() . '/style/css/all.css' ?>">
        <link rel="stylesheet" type="text/css" href="<?php echo base_url() . '/style/css/boke.css' ?>">
        <link rel="stylesheet" type="text/css" href="<?php echo base_url() . '/style/css/more.css' ?>">
        <style>
            #AID td
                {
                    width: 100px;
                    height: 18px;
                    border-right:  1px solid #D7D5C1;
                }
                a{
                    text-decoration: none;
                    color: #26241C;
                }
                a:hover{
                    color: #B0A992;
                }
        </style>
        <script type="text/javascript">
            window.onload=function(){

                var oUl=document.getElementById('ul1');
                var aLi=oUl.getElementsByTagName('li');
                var oDiv=document.getElementById('right');
                var aDiv=oDiv.getElementsByTagName('div');

                for (var i = 0; i < aLi.length; i++) {
                    aLi[i].index=i;
                    aLi[i].onclick=function(){
                        for (var i = 0; i < aLi.length; i++) {
                            aLi[i].className="";
                        }
                        this.className="active";
                        for (var i = 0; i < aDiv.length; i++) {
                            aDiv[i].style.display="none";
                        }
                        aDiv[this.index].style.display="block";
                    }   
                }
            };
           /* window.onload=function(){
                var oCh=document.getElementById('al');
                var oTab=document.getElementById('tab');
                var aCh=oTab.getElementsByTagName('oTab');

                oCh.onclick=function(){
                    for (var i = 1; i < aCh.length; i++) {
                        aCh[i].checked=true;
                    }
                };
            };*/
        </script>
    </head>
    <body>
    	<div id="header">
            <img src="<?php echo base_url().'\bg\header.jpg'?>" width="900px" height="180px">   
        </div>
    	<div id="lead">
            <ul>
                <li><a href="<?php echo site_url('home/u_look')?>">首页</a></li>
                <li><a href="<?php echo site_url('user/more_bolgs')?>">博文</a></li>
                <li><a href="<?php echo site_url('user/photo')?>">相册</a></li>
                <li><a href="<?php echo site_url('user/myself')?>">关于我</a></li>
            </ul>   
        </div>
    	<div id="main">
            <div style=" width: 873px; height: 665px; margin: 15px 11px; border: 3px dashed #D1CEB7; border-radius: 8px; background: #FFF;">
                <div id="top">
                    博文
                </div>
                <div id="left">
                    <div>分类</div>
                    <ul id="ul1">
                        <li class="active">所有博文</li>
                        <li>原创作品</li>
                        <li>英雄简介</li>
                        <li>铭文搭配</li>
                        <li>出装攻略</li>
                    </ul>
                </div>
                <div id="right">
                    <div style="display: block;">
                        <table>
                                <tr id="AID">
                                    <td style="width: 220px;">标题</td>
                                    <td>用户</td>
                                    <td style="border: none;">时间</td>
                                    <td>操作</td>
                                </tr>
                                <?php foreach($article as $v): ?>
                                    <tr>
                                        <td><?php echo $v['title']?></td>
                                        <td><?php echo $v['username']?></td>
                                        <td><?php echo date('Y-m-d-h-i-s',$v['time'])?></td>
                                        <td>[<a href="<?php echo site_url('user/article_content/'.$v['aid'])?>">查看</a>]</td>
                                    </tr>
                                <?php endforeach ?>
                        </table>
                        <table class="active1">
                                <tr style="text-align:center;"><td style="text-align:center;"><?php echo $links ?></td></tr>
                        </table>
                    </div>
                    <div>
                        <table>
                                <tr id="AID">
                                    <td style="width: 220px;">标题</td>
                                    <td>用户</td>
                                    <td style="border: none;">时间</td>
                                    <td>操作</td>
                                </tr>
                                <?php foreach($article_r as $v): ?>
                                    <tr>
                                        <td><?php echo $v['title']?></td>
                                        <td><?php echo $v['username']?></td>
                                        <td><?php echo date('Y-m-d-h-i-s',$v['time'])?></td>
                                        <td>[<a href="<?php echo site_url('user/change_article/'.$v['aid'])?>">修改</a>]</td>
                                    </tr>
                                <?php endforeach ?>
                        </table>
                    </div>
                    <div>该分类暂无博文!</div>
                    <div>该分类暂无博文！</div>
                    <div>该分类暂无博文！</div>
                </div>
            </div>
    	</div>
    </body>
</html>